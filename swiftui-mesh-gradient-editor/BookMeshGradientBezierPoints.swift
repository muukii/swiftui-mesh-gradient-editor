...
struct MeshGradientPlayground: View {
        ...
        do {
          ...
          for column in matrix.indices {
            for row in matrix[column].indices {
              ...
              matrix[column][row].point.leadingControlPoint.x *= coefficient
              matrix[column][row].point.trailingControlPoint.x *= coefficient
              matrix[column][row].point.topControlPoint.x *= coefficient
              matrix[column][row].point.bottomControlPoint.x *= coefficient
            }
          }
        }
        ...
        do {
          ...
          for column in matrix.indices {
            for row in matrix[column].indices {
              ...
              matrix[column][row].point.leadingControlPoint.y *= coefficient
              matrix[column][row].point.trailingControlPoint.y *= coefficient
              matrix[column][row].point.topControlPoint.y *= coefficient
              matrix[column][row].point.bottomControlPoint.y *= coefficient
            }
          }
  ...
  init(locations: Binding<Locations>) {
    self._points = locations
  }
  ...
  private static func handlesForPoint(
    point: Binding<MeshGradientPlayground.ControlPoint>,
    size: CGSize,
  ...
  private static func handlesForBezierPoint(
    point: Binding<MeshGradientPlayground.BezierControlPoint>,
    size: CGSize,
    isActive: Bool,
    onSelect: @escaping @MainActor () -> Void
  ) -> some View {
...
#Preview("BezierPoints") {
  @Previewable @State var points: MeshGradientPlayground.Locations = .bezierPoints(
    .init(matrix: [
      [
        .init(
          point: .init(
  ...
  return MeshGradientPlayground(locations: $points)
}

#Preview("Points") {
  @Previewable @State var points: MeshGradientPlayground.Locations = .points(
    .init(matrix: [
      [
        .init(position: .init(0, 0), color: .red),
        .init(position: .init(1, 0), color: .pink),
      ],
      [
        .init(position: .init(0, 1), color: .cyan),
